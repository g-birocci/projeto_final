...
export default function ChatPage() {
  ...
  const handleDonate = async () => {
    const productId = conversaSelecionada?.itemId?._id;
    if (!productId) {
      toast({ type: "info", message: "Produto não encontrado para esta conversa." });
      return;
    }

    if (!confirm("Tem certeza que deseja doar este produto?")) return;

    try {
      setCarregando(true);
      const receiverId = conversaSelecionada?.participants?.find((p) => p !== meuId);
      const result = await donateProduct(productId, receiverId);
      if (result.error) {
        toast({ type: "info", message: result.message || "Erro ao doar produto" });
      } else {
        toast({ type: "success", message: "Produto doado com sucesso!" });
      }
    } catch (err) {
      console.error(err);
      toast({ type: "error", message: err.message || "Erro ao doar produto" });
    } finally {
      setCarregando(false);
    }
  };
        ...
        {!conversaAtiva ? (
                    ...
                    >
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-ecodoa-primary text-sm truncate">
                          {titulo}
                          </p>
          ...
          <>
            <div className="mb-4 flex items-center gap-3">
              ...
              <Link href={`/app/products/${conversaSelecionada?.itemId?._id}`} alt={conversaSelecionada?.itemId?.title}>
                {conversaSelecionada?.itemId?.title || "Conversa"}
              </Link>
              </h2>

              {conversaSelecionada?.itemId?.ownerId === meuId && (
                <Button
                  onClick={handleDonate}
                  disabled={carregando}
                  className="bg-[var(--ecodoa-primary)] text-white hover:bg-[var(--ecodoa-olive)] text-sm px-3 py-1 rounded-md"
                >
                  {carregando ? "Doando..." : "Doar"}
                </Button>
              )}
            </div>
                ...
                {carregando && mensagens.length === 0 ? (
                  ...
                  mensagens.map((msg) => {
                    ...
                    return (
                      <div
                        key={String(msg._id)}
                        className={`max-w-[75%] p-2.5 rounded-lg ${souAutor
