...
export default function Navbar({ currentView = "home", onNavigate }) {
  ...
  const [mounted, setMounted] = useState(false); // 👈 adiciona este estado

  useEffect(() => {
    // garante que só renderiza após o cliente montar
    setMounted(true);
  }, []);
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-40 border-b border-[#9FC131]/30 bg-[#005C53]/95 backdrop-blur-md shadow-md transition-all duration-300">
      <nav className="mx-auto flex max-w-6xl items-center justify-between px-5 py-3 text-[#D6D58E] sm:px-8">
        ...
        <div className="flex items-center gap-3">
          <Image
            ...
            width={40}
            height={40}
            className="rounded-full border-2 border-[#9FC131] shadow-md hover:scale-105 transition-transform"
          />
          ...
          <button
            ...
            className="flex h-10 w-10 items-center justify-center rounded-xl border border-[#9FC131]/40 bg-[#005C53]/90 text-2xl text-[#DBF227] hover:bg-[#77850B]/40 lg:hidden"
          >
            {menuOpen ? "✖" : "☰"}
          </button>
        </div>
        ...
        >
          ...
          />
          <span className="hidden sm:inline text-xl font-bold tracking-tight text-[#DBF227]">
            EcoDoa
          </span>
        </button>
